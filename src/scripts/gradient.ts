...
import { Material, Mesh, PlaneGeometry, Uniform } from '../types'
...
interface Gradient {
  ...
  conf: { presetName: string, wireframe: boolean, zoom: number, rotation: number, playing: boolean, density: number[] }
  uniforms: {
    u_time: Uniform
...
class Gradient implements Gradient {
  constructor() {
    ...
    this.conf = { playing: false, density: [], presetName: '', rotation: 0, wireframe: false, zoom: 0 }
    this.uniforms
    this.t = 1253106
    this.last = 0
    this.width
  ...
  initGradient(selector: string) {
    ...
    throw new Error(`Did not provide canvas element, given selector = ${selector}`)
  }
  ...
  shouldSkipFrame(timestamp: number) {
    ...
    return !!window.document.hidden || (!this.conf.playing || (timestamp % 2 === 0 || false))
  }
   ...
   */
  waitForCssVars() {
    const gradientColor1 = this.computedCanvasStyle.getPropertyValue('--gradient-color-1')
  ...
  initGradientColors() {
    ...
    const expandedHexCodes = cssPropertyNames.map((cssPropertyName) => {
